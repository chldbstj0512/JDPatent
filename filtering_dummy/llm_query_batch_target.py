"""
OpenAI Batch API를 사용하여 특허 출원인(applicant)과 대상 기업(target)의 매칭을 검증하는 스크립트
...
# 입력/출력 파일 경로
#INPUT_CSV_PATH = "filtering_dummy/data/input/preprocessed_output.csv"
INPUT_CSV_PATH = "/Users/hajuheon/Task/ai/JDPatent/filtering_dummy/data/output/target_filtered.xlsx"
OUTPUT_DIR = "filtering_dummy/data/output"
TIMESTAMP = datetime.now().strftime('%Y%m%d_%H%M%S')
OUTPUT_XLSX_PATH = f"{OUTPUT_DIR}/validated_{MODEL}_batch_{TIMESTAMP}.xlsx"
BATCH_IDS_FILE = f"{OUTPUT_DIR}/batch_ids_{TIMESTAMP}.json"


def create_prompt(row: dict) -> str:
    """프롬프트 생성 - 캐싱 최적화: 정적 부분을 앞에, 동적 부분을 뒤에 배치"""
...
def create_batch_requests(rows: list, batch_num: int, start_idx: int) -> str:
    ...
    batch_file_path = f"{OUTPUT_DIR}/batch_requests_{TIMESTAMP}_part{batch_num}.jsonl"
            ...
            request = {
                ...
                "body": {
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": "당신은 한국의 글로벌 특허 데이터 매칭 검증 전문가입니다. 각국의 기업과 사업 분야에 대해서 잘 알고 있습니다. 항상 유효한 JSON 형식으로만 응답합니다."
                        },
                        {
                            "role": "user",
...
def submit_batch(file_path: str, batch_num: int) -> str:
    ...
    batch = client.batches.create(
        ...
        metadata={
            "description": f"Patent matching validation - Part {batch_num} - {TIMESTAMP}"
        }
    )
...
def resume_batches(batch_ids_file: str = None):
    ...
    사용법:
        resume_batches('filtering_dummy/data/output/batch_ids_20260114_123456.json')
    """
    ...
    if all_results:
        output_path = batch_ids_file.replace('batch_ids_', 'validated_').replace('.json', '.xlsx')
...
def main():
    ...
    print("=" * 70)
    print("OpenAI Batch API 특허 매칭 검증 (대용량 지원)")
    print("=" * 70)
    print(f"모델: {MODEL}")
